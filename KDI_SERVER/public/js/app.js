...
/*! Export depuis http://mths.be/placeholder v2.0.7 by @mathias */
...
+function ($)
{
    ...
    // background when one clicks on an item of the menu
    $(document).on('click.app.bjax.data-api', '[data-bjax], .nav-archenis a', function (e)
    {
...
+function ($)
{
    ...
    $(function()
    {
        ...
        // RESTRICT FOR JUST PAGE MENU
        $('.menupage').on('mouseenter', function ()
        {
        ...
        jQuery(window).on('load', function()
        {
            ...
            // When event resize is detected on browser, we definite a new height of these items on the page menu
            $ (window).width()>996 ? ( heightctg=$('#content').height()-180,heightplat=$('#content').height()-(66+$('.ssctg').height()+60+68) ) : (heightctg=heightplat=200);
            $('.catg').css('height',''+(heightctg)+'px');
            $('.catg').css('max-height',''+(heightctg)+'px');
            $('.plats').css('height',''+(heightplat)+'px');
            $('.plats').css('max-height',''+(heightplat)+'px');

            // Definition of the size of the content one
            heightcontent=$('#content').height()-100<500 ? 500 : $('#content').height()-100;

            // assignment of the size in the monitoring page
            $('.item-emplacement').css('height',(heightcontent-150)+'px');

            // automatic reduction of the sidebar according to the size of the device
            $ (window).width()<1300 ? $('.al-sidebar').addClass('nav-xs') : $('.al-sidebar').removeClass('nav-xs') ;

        });

        // REDEFINES ITEMS SIZE
        jQuery(window).on('resize', function (event)
        {

            // When event resize is detected on browser, we definite a new height of these items on menu page
            $ (window).width()>996 ? ( heightctg=$('#content').height()-180,heightplat=$('#content').height()-(66+$('.ssctg').height()+60+68) ) : (heightctg=heightplat=200);
            $('.catg').css('height',''+(heightctg)+'px');
            $('.catg').css('max-height',''+(heightctg)+'px');
            $('.plats').css('height',''+(heightplat)+'px');
        ...
        toastr.options.onShown = function() { console.log('Affichage'); }
        toastr.options.onHidden = function() { console.log('Disparition'); }
        toastr.options.onclick = function() { console.log('Cliqué'); }
        toastr.options.onCloseClick = function() { console.log('Clique sur le boutton close'); }

        toastr.info( window.location.href,'Bienvenue sur ');
...
var updatepanelcmd=0;
// section add cmd
function showaddcmd()
{
    updatepanelcmd==0 ? ( $('#paneladdcmd').addClass('show'), $('#panelviewcmd').addClass('hide'),updatepanelcmd=1 ) : ( $('#paneladdcmd').removeClass('show'), $('#panelviewcmd').removeClass('hide'),updatepanelcmd=0 ) ;
}
    ...
    toastr.options =
        {
            ...
            /* A utiliser pour les suppressions*/
            // "showDuration": "300",
            // "hideDuration": "1000",
            // "timeOut": 0,
            // "extendedTimeOut": 0,
