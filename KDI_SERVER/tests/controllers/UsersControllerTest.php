...
class UsersControllerTest extends TestCase
{
    ...
    {
        $admin = Surface::where('name', 'Administrator')->firstOrFail();
    ...
    {
        $user = Surface::where('email', 'user@localhost.com')->first();
        if (is_null($user)) {
            $user = Surface::create([
        ...
        $authority->allow('destroy', Surface::class, function ($self, $user) {
            return $self->user()->id === $user->id;
        });
        $this->assertTrue($authority->can('destroy', new Surface(['id'=> $user->id])));
        $this->assertTrue($authority->cannot('destroy', new Surface));
    }
